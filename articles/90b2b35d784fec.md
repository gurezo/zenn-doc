...
- メジャーバージョンは、**必ず一つずつ** 上げる
  - ex. Angular v19.x => Angular v20.x
...
![ex. Angular v19.x => Angular v20.x](/images/select.version.png)
_ex. Angular v19.x => Angular v20.x_
...
Angular v19.x => Angular v20.x の場合、**_基本的には_**アップデート前に実施する作業はありません。
...
### v20 のマイナーバージョンまで指定して、アップデートする場合

```sh
$ ng update @angular/core@v20.0.0 @angular/cli@v20.0.0
```

### Angular CDK, Angular Materail をアップデートする場合

```sh
$ ng update @angular/cdk@v20.x @angular/material@v20.x
```

### 先行バージョンで、アップデートする場合

- 先行バージョン（ex v20.1.0-next.0 or  v20.1.0-rc.0）ですので、参考まで
...
- 「–no-save」とするのは、これを指定しないと package.json が作成されてしまいます。
  ...
  $ mkdir test-project
  $ cd test-project
  $ npm i @angular/cli@v20.x --no-save
  ```
