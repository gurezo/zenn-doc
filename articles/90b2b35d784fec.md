...
:::message
[公式ガイド](https://angular.dev/update-guide?v=20.0-21.0&l=1)の手順を踏むことによって、回り道せずにアップデートが出来ます。
:::

- [`公式ガイド`](https://angular.dev/update-guide) で、手順を確認
- メジャーバージョンは、**必ず一つずつ** 上げる
  - ex. Angular v21.x => Angular v22.x

# アップデート手順

- [公式ガイド](https://angular.dev/update-guide?v=21.0-22.0&l=1) を参照
- バージョンを指定
...
![ex. Angular v21.x => Angular v22.x](/images/select.version.png)
_ex. Angular v21.x => Angular v22.x_
...
Angular v21.x => Angular v22.x の場合、**_基本的には_** アップデート前に実施する作業はありません。
...
### v22 のマイナーバージョンまで指定して、アップデートする場合

```sh
$ ng update @angular/core@v22.0.0 @angular/cli@v22.0.0
```

### Angular CDK, Angular Materail をアップデートする場合

```sh
$ ng update @angular/cdk@v22.x @angular/material@v22.x
```

### 先行バージョンで、アップデートする場合

- 先行バージョン（ex v22.1.0-next.0 or  v22.1.0-rc.0）ですので、参考まで
...
- サポート期間については、下記を参照して下さい
  - [アクティブにサポートされているバージョン](https://angular.jp/reference/versions#actively-supported-versions)
  - [サポートされていない Angular バージョン](https://angular.jp/reference/versions#unsupported-angular-versions)
...
- [v21.x => v22.x ファイル差分](https://github.com/gurezo/ng-migrate-sample/pull/102/files)
- [v20.x => v21.x ファイル差分](https://github.com/gurezo/ng-migrate-sample/pull/101/files)
- [v19.x => v20.x ファイル差分](https://github.com/gurezo/ng-migrate-sample/pull/100/files)
...
- 「–no-save」とするのは、これを指定しないと package.json が作成されてしまいます。
  ...
  $ mkdir test-project
  $ cd test-project
  $ npm i @angular/cli@v22.x --no-save
  ```
